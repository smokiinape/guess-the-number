...
// guessCounter = 0;
let radnomNum = Math.floor(Math.random() * 50);
guessCounter = 0;
...
guessBtn.addEventListener('click', () => {
    ...
    let input = document.querySelector('#input').value;
    if (input < 1 || input > 50) {
        h1.innerHTML = 'Please enter a number between 1- 50';
        ...
        h1.innerHTML = 'Number must be between 1-50';

    } else {

        alert('Please enter a number noob');
        location.reload();
    }
});
...
function guessIncrement(guessCounter) {
    if (guessCounter === 5) {
        guessBtn.setAttribute('disabled', '');
        h1.textContent = 'You lost! play again pleb!';
